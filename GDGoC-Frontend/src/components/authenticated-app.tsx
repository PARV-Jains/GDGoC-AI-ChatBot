...
const AuthenticatedCore = ({ user, onLogout }: AuthenticatedAppProps) => {
  ...
  const handleNewChannel = async () => {
    ...
    const newChannel = client.channel("messaging", uuidv4(), {
      name: `New Writing Session ${new Date().toLocaleTimeString()}`,
      members: [user.id],
    });
  ...
  const handleNewChatMessage = async (message: { text: string; file?: File  }) => {
    if (!user.id) return;

    try {
      // 1. Create a new channel with the user as the only member
     const channelName = message.text ? message.text.substring(0, 50) : "New Image Chat";
    const newChannel = client.channel("messaging", uuidv4(), {
      name: channelName,
      members: [user.id],
    });
    await newChannel.watch();
      ...
      await memberAddedPromise;
       const attachments = [];
       if (message.file) {
      // Upload the image to Stream's CDN
      const response = await newChannel.sendImage(message.file);
      // Create an attachment object with the returned URL
      attachments.push({
        type: "image",
    ...
    <div className="flex h-full w-full">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <ChatInterface
          ...
          onNewChannelRequest={handleNewChannel}
